jQuery(document).ready(function($) {
	...
	jQuery('#the-list td.status.column-status').each(function(){
		...
		if (installed_on === "Not Installed") {
			...
			jQuery(".alignleft.actions.bulkactions select").first().val('tgmpa-bulk-install');
			}
		...
		if (installed_on === "Installed But Not Activated") {
			...
			jQuery(".alignleft.actions.bulkactions select").first().val('tgmpa-bulk-activate');
			}

		}
	});
	jQuery("#cb-select-all-1").attr("checked", "on");
